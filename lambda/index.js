...
function getDateUtc(date, format){  
  console.log('[getDateUtc date] => ' + date);
  const timezone = TIMEZONE;
  var localTime  = moment.utc(date).toDate();
  const valor = moment.tz(localTime, timezone).format(format);
  console.log('[getDateUtc valor] => ' + valor);
  return valor;
}
...
const AssociarContaIntentHandler = {
    canHandle(handlerInput) {
...
const ContaIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    const speakOutput = resAccount.nickname + ', seu saldo é de R$ ' + resBalance.amount.replace('.',',') + ' e você possui linha de crédito disponível de R$ ' +
